...
export async function POST(req: NextRequest) {
  ...
  const salida =
    phaseId === "fase_6"
      ...
      : `# INSTRUCCIÓN DE SALIDA\nCuando el cliente apruebe el contenido de esta fase, o cuando tú consideres que está listo para aprobación, llama a la tool \`propose_section\` con el JSON según el schema. Tras llamarla con éxito, avisa al cliente en un mensaje breve que revise la tarjeta de propuesta y use los botones Aprobar o Pedir cambios.\n# DISCIPLINA DE EDICIÓN (cuando el cliente pide cambios sobre la propuesta existente)\n1. DIFF MÍNIMO: reenvía el JSON COMPLETO cambiando EXCLUSIVAMENTE lo que el cliente pidió; todo lo demás se preserva IDÉNTICO, palabra por palabra. PROHIBIDO retocar, "mejorar", renombrar o reordenar nada que no se pidió.\n2. Si la tool devuelve ok=false por errores en partes que TÚ alteraste sin que te lo pidieran: restaura esas partes a su versión original y reintenta con el diff mínimo.\n3. Si la tool devuelve ok=false y el error recae sobre EXACTAMENTE lo que el cliente pidió: NO lo sustituyas por otra versión, NO persistas nada distinto de lo pedido. Responde empezando literalmente por "No apliqué el cambio porque: [la razón en lenguaje simple]" y di qué haría falta para poder aplicarlo (p. ej. confirmar esa cifra como caso real del Credibility Bank, o dejarla como [X] hasta tener el dato).\n4. PROHIBIDO decir "hecho", "listo", "actualizado" o "lo incorporo" si la tool no devolvió ok=true conteniendo el cambio EXACTO que el cliente pidió.`;
